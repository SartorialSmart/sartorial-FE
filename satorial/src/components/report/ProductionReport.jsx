...
const ProductionReport = () => {
  ...
  const fetchData = async (filter, startDate, endDate) => {
    ...
    try {
      ...
      const [byStaffRes, summaryRes] = await Promise.all([
        ProductionService.getReportByStaff(params),
        ProductionService.getReport(params),
      ]);

      const staffArr =
        byStaffRes?.data ||
        byStaffRes?.results ||
        (Array.isArray(byStaffRes) ? byStaffRes : []);
      const summaryData = summaryRes?.data || summaryRes || {};

      const rows = (Array.isArray(staffArr) ? staffArr : []).map((row) => ({
        name:
          row.staff_name ||
          `${row.first_name || ""} ${row.last_name || ""}`.trim() ||
          row.name ||
          "Unknown",
        role: row.role || row.staff_role || "",
        assigned: row.total_assigned ?? row.assigned_units ?? "-",
        completed: row.completed_units ?? row.completed ?? "-",
        ongoing: row.in_progress_units ?? row.in_progress ?? "-",
        onTime:
      ...
      setStaffRows(rows);
      setSummary({
        totalOrders: summaryData.total_orders ?? summaryData.orders_count ?? 0,
        totalUnits: summaryData.total_units ?? 0,
        completedOrders: summaryData.completed_orders ?? 0,
        completedUnits: summaryData.completed_units ?? 0,
        inProgressOrders: summaryData.in_progress_orders ?? 0,
        qaOrders: summaryData.qa_orders ?? 0,
        staffInvolved: summaryData.staff_involved ?? rows.length,
      });
    } catch {
  ...
  }, [staffRows, searchQuery, selectedRole]);
  ...
  const handleExport = useCallback(() => {
    ...
    XLSX.writeFile(
      wb,
      `production_report_${new Date().toISOString().slice(0, 10)}.xlsx`
    );
  }, [filteredRows]);
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-gray-600">
              Track production output, staff performance and on-time completion
            </p>
          </div>
      ...
      <div className="mb-4 flex items-center justify-between">
        <div className="text-sm text-gray-600">
          ...
          of <span className="font-semibold text-gray-900">{staffRows.length}</span>{" "}
          staff
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-gray-600">Loading production report...</p>
          </div>
        ) : error ? (
                      ...
                      <div className="flex flex-col items-center gap-3 text-gray-400">
                        ...
                        <p className="text-sm">
                          {searchQuery || selectedRole !== "All"
                            ...
                            : "No production assignments in this period"}
                        </p>
                      </div>
                    </td>
                  </tr>
