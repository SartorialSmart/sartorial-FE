...
const MonthlyDataReport = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setAllOrdersData(data.map((item) => ({ month: item.month, value: 0 })));
      } catch {
  ...
  useEffect(() => {
    ...
    } else if (selectedFilter === "This Year" || selectedFilter === "Custom Date") {
      const year = parseInt(customYear);
      setSalesData(allSalesData.filter((d) => d.month && d.month.includes(String(year))));
      setExpensesData(allExpensesData.filter((d) => d.month && d.month.includes(String(year))));
      setOrdersData(allOrdersData.filter((d) => d.month && d.month.includes(String(year))));
    }
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      {(selectedFilter === "This Year" || selectedFilter === "Custom Date") && (
