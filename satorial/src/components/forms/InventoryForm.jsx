  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="pt-4 border-t border-gray-100">
        ...
        {form.unit_cost && form.selling_price && parseFloat(form.selling_price) > 0 && (
          <div className="mt-2 text-xs text-gray-500">
            Profit margin:{" "}
            <span className={`font-semibold ${
              parseFloat(form.selling_price) >= parseFloat(form.unit_cost)
                ? "text-green-600"
                : "text-red-600"
            }`}>
              {((parseFloat(form.selling_price) - parseFloat(form.unit_cost)) / parseFloat(form.unit_cost) * 100).toFixed(1)}%
            </span>
            {" "}(₦{(parseFloat(form.selling_price) - parseFloat(form.unit_cost)).toFixed(2)} per unit)
          </div>
        )}
      </div>
